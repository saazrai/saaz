...
const loginWithGoogle = () => {
    try {
        const redirectUrl = typeof route !== 'undefined' ? route('auth.social.redirect', 'google') : '/auth/social/google/redirect';
        window.location.href = redirectUrl;
    } catch (error) {
        console.error('Error redirecting to Google OAuth:', error);
        alert('Unable to redirect to Google login. Please try again or use email/password login.');
    }
};
...
<template>
    ...
    <div class="min-h-screen flex items-center justify-center px-4">
        <div class="w-full max-w-md bg-white dark:bg-gray-800 rounded-2xl shadow-lg p-8">
            <h2 class="text-2xl font-bold text-center text-blue-900 dark:text-blue-100">Welcome Back!</h2>
